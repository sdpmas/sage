...
def predict_fn(input_data, model):
    ...
    query_repr=model.search_model.get_representation_one('NL: '+input_data,device=device)
    # codes=model.indices.get_nns_by_vector(query_repr,3)
    ex_code_idx,_=model.search_model.searcher.search(query_repr.detach().cpu().numpy())
    # ex_code_idx=model.search_model.searcher.get_nns_by_vector(query_repr,5)
    ex_codes=[model.search_model.codebase[c]['code'] for c in ex_code_idx]
    filtered_ex_codes=[]
    query_prompt="Query:\n"+input_data+'\n'
    ...
    if ex_codes:
        for i_e_code,e_code in enumerate(ex_codes):
            if i_e_code>1:
                filtered_ex_codes.append(remove_comments_and_docstrings(e_code))
                break

            try:
                f_e_code=remove_comments_and_docstrings(e_code)
                filtered_ex_codes.append(f_e_code)
    ...
    total_query_ids=query_ids+context_ids+excode_ids
    add_newline=False 
    #dynamically add block size
    if len(total_query_ids)>400:
        add_newline=True
    total_query_ids=total_query_ids[:400]
    if not add_newline:
    ...
    decoded_out=model.gen_tokenizer.decode(output_ids).split("Generate Code:\n")[1].replace("<|endoftext|>", "")
    return_dict={'code':decoded_out,'examples':filtered_ex_codes}
    return return_dict
