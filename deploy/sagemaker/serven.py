...
def model_fn(model_dir):
    ...
    py_gen_model=transformers.GPTNeoForCausalLM.from_pretrained("EleutherAI/gpt-neo-125M")
    js_gen_model=transformers.GPTNeoForCausalLM.from_pretrained("EleutherAI/gpt-neo-125M")
    seargen_model=SearchGenModel(search_model=search_model,py_gen_model=py_gen_model,js_gen_model=js_gen_model,gen_tokenizer=gen_tokenizer)
...
def predict_fn(input_data, model):
    ...
    with torch.no_grad():
        if input_data['language']=='python':
            full_query='Language: Python'+' NL: '+input_data['query'].strip()
        else:
            full_query='Language: Javascript'+' NL: '+input_data['query'].strip()

        query_repr=model.search_model.get_representation_one(full_query,device=device)
    # codes=model.indices.get_nns_by_vector(query_repr,3)
#     ex_code_idx=[]
    ex_code_idx,_=model.search_model.searcher.search(query_repr.detach().cpu().numpy(), final_num_neighbors=7)
    # ex_code_idx=model.search_model.searcher.get_nns_by_vector(query_repr,5)
    ex_codes=[{'code':model.search_model.codebase[c]['code'],'url':model.search_model.codebase[c]['url']} for c in ex_code_idx if model.search_model.codebase[c]['language']==input_data['language']]
    filtered_ex_codes=[]
    query_prompt="Query:\n"+input_data['query']+'\n'
    ...
    if input_data['language']=='python':
        current_gen_model=model.py_gen_model
    else:
        current_gen_model=model.js_gen_model
    with torch.no_grad():
        output_ids=current_gen_model.generate(
                        input_ids,
                        num_beams=2,
                        early_stopping=True,
...
#     request_body=json.dumps({'query':'concatenate two dicts','context':'print(code)','language':'python'})
